...
public class Canteen_service {
		...
		public boolean QuailityRequest(Canteen_Request canteen_Request){
			...
			canteen_Request.getRequest_info().setCR_id(idFactory.createRequestId());//配置id
			boolean r = c_Request_dao.setRequest(canteen_Request.getRequest_info())&&canteen_dao.addCanteen(canteen_Request.getCanteen())
		...
		public boolean CreateFood(Food food){
			food.getFood_info().setF_id(idFactory.createFoodId());//id工厂问题
			return food_dao.setFood(food.getFood_info());
			//创建菜品
		}
		...
		public boolean ChangeFood(Food food){
			return food_dao.ChangeFood(food.getFood_info());
			//修改菜品
		}
...
//		}
		...
		public Food createFood(Food_Bean data) {
			...
			food.getFood_info().setF_type(foodtype_dao.getFoodTypeName(data.getF_type()));//数据库需要修改
			food.getFood_info().setF_uptime(data.getF_uptime());
			food.setNorms(norms_dao.queryList(food.getFood_info().getF_id()));//数据库修改
			return food;
		}
